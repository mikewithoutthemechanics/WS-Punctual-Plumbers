    ...
    >
      <style>{`
        ...
        * { scrollbar-width: thin; scrollbar-color: #00D2FF33 #0b1b2b;}
        ...
        input[type=range] { accent-color: #00D2FF; min-height: 44px; touch-action: pan-y; }
        input[type=range]::-webkit-slider-thumb { min-width: 44px; min-height: 44px; }
        input[type=range]::-moz-range-thumb { min-width: 44px; min-height: 44px; }
        button, a { touch-action: manipulation; min-height: 44px; min-width: 44px; }
        .touch-carousel { touch-action: pan-y pinch-zoom; }
      ...
      >
        <div className="mx-auto max-w-[1250px] px-4 sm:px-7 pt-4">
          <div className="glass rounded-2xl px-4 sm:px-6 py-[13px] flex items-center justify-between shadow-[0_12px_60px_rgba(0,0,0,0.38)]">
            ...
            <div className="hidden lg:flex items-center gap-8 text-[13.5px] text-white/80 font-[500]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={()=>setMobileMenu(!mobileMenu)} className="lg:hidden ml-1 px-3 py-2 text-white/80 text-[22px] min-w-[44px] min-h-[44px] flex items-center justify-center">≡</button>
            </div>
          </div>
        </div>
        <AnimatePresence>
          {mobileMenu && (
            <motion.div
              ...
              exit={{ opacity:0, y:-8 }}
              className="lg:hidden mx-4 mt-2 glass rounded-2xl px-4 py-4 text-[14px] text-white/85"
            >
              {["Services","Process","Coverage","Reviews","Journal"].map(l=>(
      ...
      <section ref={heroRef} className="relative h-[100svh] min-h-[740px] w-full overflow-hidden">
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative z-10 h-full mx-auto max-w-[1250px] px-5 sm:px-7 flex flex-col justify-start lg:justify-center pt-[260px]">
          <motion.div
            initial={{ opacity:0, y:34 }}
            animate={{ opacity:1, y:0 }}
            transition={{ delay:0.22, duration:0.9, ease:[0.22,1,0.36,1] }}
            className="mt-8 sm:mt-10 max-w-[980px]">
